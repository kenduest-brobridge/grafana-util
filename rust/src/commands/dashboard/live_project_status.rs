...
pub(crate) fn build_live_dashboard_domain_status(
    ...
    let mut signal_keys = DASHBOARD_SIGNAL_KEYS
        ...
        .collect::<Vec<String>>();
    for signal_key in details.warning_signal_keys {
        if !signal_keys.iter().any(|item| item == &signal_key) {
            signal_keys.push(signal_key);
        }
    }

    let (status, reason_code, mut next_actions) = if details.dashboard_count == 0 {
        (
    ...
    } else if details.datasource_count == 0 {
        (
            PROJECT_STATUS_PARTIAL,
    ...
    if !details.warnings.is_empty() && status == PROJECT_STATUS_READY {
        next_actions.extend(
    ...
    StatusReading {
        ...
        primary_count: details.dashboard_count,
        ...
        signal_keys,
        blockers: Vec::new(),
        warnings: details.warnings.into_iter().map(Into::into).collect(),
        next_actions,
