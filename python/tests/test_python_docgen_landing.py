...
class DocgenLandingTests(unittest.TestCase):
    def test_load_landing_page_parses_english_source(self):
        ...
        self.assertEqual(page.search.title, "Start with a task")
        self.assertEqual(len(page.sections), 3)
        self.assertEqual(page.sections[0].title, "Quick Start")
        self.assertEqual(page.sections[0].tasks[0].title, "Export dashboards")
        self.assertEqual(page.sections[0].tasks[0].links[0].target, "../commands/en/dashboard-export.md")
