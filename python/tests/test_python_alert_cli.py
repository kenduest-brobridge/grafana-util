...
CLIENT_MODULE_PATH = PYTHON_ROOT / "grafana_utils" / "clients" / "alert_client.py"
PROVISIONING_MODULE_PATH = PYTHON_ROOT / "grafana_utils" / "alerts" / "provisioning.py"
...
class AlertUtilsTests(unittest.TestCase):
    ...
    def test_alert_list_contact_points_with_org_id_renders_org_fields_in_json(self):
        args = alert_utils.parse_args(
            ["list-contact-points", "--org-id", "7", "--json"]
        )
        args.username = "admin"
        args.password = "secret"
    ...
    def test_alert_build_contact_point_export_document_strips_server_managed_fields(
        self,
    ):
    ...
    def test_alert_build_import_operation_accepts_legacy_tool_document_without_schema_version(
        self,
    ):
        document = alert_utils.build_rule_export_document(sample_rule())
    ...
    def test_alert_import_alerting_resources_updates_existing_contact_point_when_requested(
        self,
    ):
        args = alert_utils.parse_args(["--import-dir", "unused", "--replace-existing"])
        fake_client = FakeAlertClient(contact_points=[sample_contact_point()])
    ...
    def test_alert_import_alerting_resources_updates_existing_mute_timing_when_requested(
        self,
    ):
        args = alert_utils.parse_args(["--import-dir", "unused", "--replace-existing"])
    ...
    def test_alert_import_alerting_resources_updates_existing_template_when_requested(
        self,
    ):
        args = alert_utils.parse_args(["--import-dir", "unused", "--replace-existing"])
    ...
    def test_alert_import_alerting_resources_rejects_existing_template_without_replace(
        self,
    ):
        args = alert_utils.parse_args(["--import-dir", "unused"])
        fake_client = FakeAlertClient(templates=[sample_template()])
