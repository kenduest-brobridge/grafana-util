...
def _query_family(query: dict[str, Any]) -> str:
    ...
    return str(
        query.get("datasourceFamily") or query.get("datasourceType") or ""
    ).strip()
        ...
        dashboards.append(
            {
                ...
                "variableNames": sorted(
                    _normalize_string_set(record.get("variableNames"))
                ),
                "datasourceVariables": sorted(
...
def _merge_dashboard_context(
    ...
    for record in list(governance_context or []) + list(fallback_context or []):
        ...
        if (
            str(record.get("dashboardTitle") or "").strip()
            and not current["dashboardTitle"]
        ):
        ...
        current["datasourceVariableRefs"].update(
            record.get("datasourceVariableRefs") or []
        )
    return [
        {
...
def evaluate_dashboard_governance_policy(
    ...
    if version != 1:
        raise ValueError(
            "Unsupported dashboard governance policy version: %s" % version
        )
    ...
    forbid_unknown = _normalize_bool(
        datasource_policy.get("forbidUnknown"), default=False
    )
    ...
    max_queries_per_panel = _normalize_optional_int(
        query_policy.get("maxQueriesPerPanel")
    )
    ...
    forbid_select_star = _normalize_bool(
        query_policy.get("forbidSelectStar"), default=False
    )
    ...
    fail_on_warnings = _normalize_bool(
        enforcement_policy.get("failOnWarnings"), default=False
    )
    ...
    dashboard_counts: dict[tuple[str, str], int] = {}
    dashboard_complexity_scores: dict[tuple[str, str], int] = {}
    dashboard_folder_paths: dict[tuple[str, str], str] = {}
    panel_counts: dict[tuple[str, str, str, str], int] = {}
    for query in queries:
        dashboard_counts[_dashboard_key(query)] = (
            int(dashboard_counts.get(_dashboard_key(query), 0)) + 1
        )
        panel_counts[_panel_key(query)] = (
            int(panel_counts.get(_panel_key(query), 0)) + 1
        )
        ...
        dashboard_complexity_scores[_dashboard_key(query)] = (
            int(dashboard_complexity_scores.get(_dashboard_key(query), 0))
            + complexity_score
        )
        ...
        if allowed_families and family not in allowed_families:
            violations.append(
                _build_finding(
                    ...
                    "Datasource family %s is not allowed by policy."
                    % (family or "unknown"),
                    query,
                )
            )
        ...
        if (
            forbid_select_star
            and _is_sql_query(query)
            and re.search(r"\bselect\s+\*", query_text, flags=re.IGNORECASE)
        ):
            violations.append(
                _build_finding(
                    "error",
        ...
        if (
            require_sql_time_filter
            and _is_sql_query(query)
            and not _query_uses_time_filter(query)
        ):
            violations.append(
                _build_finding(
                    "error",
    ...
    if max_queries_per_panel is not None:
        for panel_key, query_count in sorted(panel_counts.items()):
            if query_count <= max_queries_per_panel:
                continue
            dashboard_uid, dashboard_title, panel_id, panel_title = panel_key
            violations.append(
                _build_finding(
                    "error",
            ...
            if (
                ...
                violations.append(
                    _build_finding(
                        ...
                        "Library panel %s is not allowed by policy."
                        % library_panel_uid,
                        extra={
                            "dashboardUid": dashboard_uid,
                            "dashboardTitle": dashboard_title,
        ...
        warnings.append(
            {
                ...
                "message": str(
                    record.get("recommendation") or str(record.get("detail") or "")
                ).strip(),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--policy", required=True, help="Path to the governance policy JSON."
    )
    parser.add_argument(
        "--governance",
        required=True,
