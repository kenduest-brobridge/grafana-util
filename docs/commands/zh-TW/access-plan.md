...
目前實作支援 `user`、`org`、`team`、`service-account` bundles。`--resource all` 先保留給後續 aggregate layer。

## 重要參數
- `--input-dir`: 要 review 的本地 access export bundle。
- `--resource`: 選擇 `user`、`team`、`org`、`service-account` 或 `all`。目前 planner 支援具體 resource，會拒絕 `all`。
...
- **After**: `access plan` 會輸出單一 review document，用穩定 action rows 表示 user、org、team 與 service-account reconcile 結果。
...
- 如果命令回報 selected resource 尚未支援，改用具體 resource，不要用 `--resource all`
