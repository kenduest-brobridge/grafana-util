...
- `--include-folder-permissions`: 從 `raw/permissions.json` 納入 folder 權限 drift review。
- `--folder-permission-match`: 選擇 `uid` 或 `uid-then-path`。預設是 `uid`；`uid-then-path` 只會在 exported UID 找不到時 fallback 到 folder path。
...
```bash
# 納入 read-only folder 權限 drift rows。
...
- JSON 輸出保留穩定 `actionId`、status、changed fields、target evidence、dependency hints 與 review hints，可供 CI 或後續 TUI 使用。Folder permission row 也會包含 subject、permission 與 inherited 標記。
...
- 如果 folder permission row 被 blocked，確認 target 有 exported folder UID，或改用 `--folder-permission-match uid-then-path`
