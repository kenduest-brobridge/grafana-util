...
## 用途
將儀表板匯出成適合 Git 管理的 `raw/`、`prompt/` 與 `provisioning/` 檔案，必要時也可附帶逐版本 `history/` 成品。

## 何時使用
當您需要一個本地匯出樹，供後續匯入、檢視、比對、Git review 或檔案 provisioning 工作流程使用時，使用這個指令。若您也需要每個匯出 org 範圍各自的版本歷史成品，請加上 `--include-history`。`prompt/` 路徑是給 Grafana UI 匯入用，不是給 dashboard API 匯入用；如果您只有一般或 raw 的 dashboard JSON，需要先轉成 prompt JSON，請使用 `dashboard convert raw-to-prompt`。
...
- `--include-history`：把逐版本 dashboard 歷史成品寫到每個匯出 org 範圍下的 `history/<uid>/vN.history.json`。
...
- Dashboard JSON 檔名使用穩定的 dashboard UID，例如 `raw/Platform/Team/Infra/cpu-main.json`，因此 title 變更只會改內容，不會造成檔案 rename。
...
- `--flat` 會把檔案直接寫在各變體目錄下。
- `--include-history` 會在每個匯出 org 範圍下加上逐版本 `history/<uid>/vN.history.json` 檔案。
...
- 如果有加 `--include-history`，每個匯出 org 範圍下都會有對應的逐版本 `history/<uid>/vN.history.json` 樹
