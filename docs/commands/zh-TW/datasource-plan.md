...
- `--output-format`: 選擇 `text`、`table`、`json` 或 `interactive`。
...
```bash
# 用互動式 review browser 檢視 action details、blockers、hints 與安全 diff。
...
- JSON 輸出保留穩定 `actionId`、status、target evidence、changed fields 與 review hints，適合 CI review；interactive 輸出則用同一個 action model 開啟唯讀終端機瀏覽器。
...
- interactive output 可讓 operator 在選擇 apply path 前檢查 blockers、warnings 與安全欄位 diff
