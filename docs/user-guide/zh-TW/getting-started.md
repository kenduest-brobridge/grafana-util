...
若要對照本章提到的旗標，請一併參考 [config profile](../../commands/zh-TW/profile.md) 與 [指令詳細說明總索引](../../commands/zh-TW/index.md)。
...
```
`observe overview live` 是共用 observe live 路徑的人類導向包裝。要看可讀摘要可用 `--output-format yaml`，想進互動式工作台就用 `--output-format interactive`。
