...
`--workspace` 是最短路徑，因為 `change` 會先嘗試在目前 repo 或工作目錄裡找常見 staged inputs，包含 Git Sync workspace root。若這不符合你的目錄布局，再改用 `--desired-file`、`--dashboard-export-dir`、`--alert-export-dir`、`--source-bundle`、`--target-inventory` 這些明確旗標。

### 1. 變更檢視 (Change Inspect)
先看目前變更包的高階摘要與輸入形狀。
```bash
# 用途：先從 Git Sync workspace root 自動發現常見 staged inputs。
