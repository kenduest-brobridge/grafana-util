...
- Shared browser detail projection helpers now live in `common/browser/detail.rs`
  and shared browser behavior tests now live in `common/browser/session_tests.rs`.
  This keeps `session.rs` focused on search state, terminal session setup, and
  the runtime event loop, clearing the architecture warning-threshold list.
