...
- Older entries moved to [`ai-changes-archive-2026-04-16.md`](/Users/kendlee/work/grafana-utils/docs/internal/archive/ai-changes-archive-2026-04-16.md).

## 2026-04-16 - Add dashboard export layout repair
- Summary: added the formal `grafana-util dashboard convert export-layout` command for repairing old dashboard export artifacts whose raw/prompt files were flattened under leaf folder titles. The repair planner uses `raw/folders.json` plus stable export index metadata, falls back safely when raw dashboard JSON omits `meta.folderUid`, reports unindexed files through `extraFiles`, supports dry-run text/table/json/yaml output, copy-mode output, and in-place repair with backups. New dashboard exports now record `folderUid`, `folderTitle`, and full `folderPath` in root and variant indexes.
- Tests: added parser/validation coverage for the new convert command, runtime tests for nested-folder planning, root-index fallback, unindexed-file reporting, repaired copy output/index updates, in-place backup behavior, and export index folder identity metadata.
- Test Run: `cargo fmt --manifest-path rust/Cargo.toml --all`; `cargo fmt --manifest-path rust/Cargo.toml --all --check`; `cargo test --manifest-path rust/Cargo.toml --quiet export_layout`; `cargo test --manifest-path rust/Cargo.toml --quiet dashboard_export_indexes_include_folder_identity_metadata`; `cargo test --manifest-path rust/Cargo.toml --quiet parser_surface_rust_tests`; `cargo test --manifest-path rust/Cargo.toml --quiet help_rust_tests`; `cargo test --manifest-path rust/Cargo.toml --quiet`; `cargo clippy --manifest-path rust/Cargo.toml --all-targets -- -D warnings`; `make man`; `make html`; `make quality-docs-surface`; `make quality-ai-workflow`; `make man-check`; `make html-check`; `git diff --check`; real export smoke: `grafana-util dashboard convert export-layout --input-dir /Users/kendlee/work/scsb/grafana-dashboard/scsb-dev/dashboards --output-dir /tmp/grafana-export-layout-test.9JfjGR/fixed --overwrite --output-format json`.
